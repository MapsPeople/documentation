...
* Ability to configure the position of labels as right (old default), below (new default), left or top of icon.

#### Changed

* The default position of labels have changed from being to the right of the icons to below.
If you want to keep the old default you can do so by applying `MPMapsIndoors.shared.displayRuleFor(displayRuleType: .main)?.labelStylePosition = .right` after loading MapsIndoors.
...
* MISDKIOS-2112-venuehandler-crash ???
* MIAIOS-1698-google-directionsrenderer-crash ???
* MISDKIOS-2098_multiple_mapcontrols ???
