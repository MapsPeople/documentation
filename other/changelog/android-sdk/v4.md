...
* **Camera control for `MapControl.goTo`.** `goTo` now takes an optional `MPCameraBehavior`
  describing how the camera should move, and an optional listener that fires once the camera
  has settled.
    * `MapControl.goTo(entity, behavior, onCameraEvent)`. Both trailing parameters are optional
      from Java as well as Kotlin, so existing `goTo(entity)` calls are unchanged.
    * `MPCameraBehavior.Builder` offers `setAnimate(animate)`,
      `setAnimationDuration(animationDuration)`, `setPadding(padding)`, `setZoomToFit(zoomToFit)`,
      `setAllowFloorChange(allowFloorChange)` and `setMaxZoom(maxZoom)`. `MPCameraBehavior.DEFAULT`
      is the behaviour used when none is given.
    * `onCameraEvent` takes an `MPCameraEventListener` that is called when the camera stops, so an
      app can sequence work after the move instead of guessing at an animation duration.

### Deprecated

* `MapControl.goTo(entity, maxZoom)` in favour of `goTo(entity, MPCameraBehavior)`. The equivalent
  is `goTo(entity, MPCameraBehavior.Builder().setMaxZoom(maxZoom).build())`. From Java,
  `goTo(entity, null)` is now ambiguous — write `goTo(entity)` or `goTo(entity, (Double) null)`.

### Fixed

* Serialising an `MPLocation`, `MPDisplayRule`, `MPPoint` or `MPVenue` with a reflective JSON
  serializer could fail with a `StackOverflowError` or an Android hidden-API error instead of
  producing JSON. Venues returned by `MapsIndoors.getVenues()` were affected.
* Setting, replacing or clearing a route while the map was still drawing the previous one could
  crash with a `ConcurrentModificationException`, and a render pass could draw a route that had
...
* `MPLocation.getRestrictions()` is now public. It returns the app user role ids (`MPUserRole.getKey()`) a location is restricted to, or `null`/empty when the location carries no restriction of its own. Note that locations restricted to roles which are not applied via `MapsIndoors.applyUserRoles()` are never loaded, so any location you can reach is already accessible to the current roles; and a location inheriting its restriction from its type carries no value here. Unrelated to `MPRoute.getRestrictions()`, which holds route avoids.

### Changed

* Location restrictions are now read from the location's `properties` payload, which is where the backend delivers them. A top-level `restrictions` field is still honoured when the properties carry none, and is still emitted when serializing, so platform wrappers that read it are unaffected.

### Fixed

* Outside-to-venue routing no longer fails intermittently with `ROUTING_ENTRY_POINTS_ERROR`. Entry points are resolved from the routing graph alone; previously the query also required the route layers, which arrive on a separate request, so a route requested in the window between the two responses would fail. Route layers are only used by the offline path optimiser and were never needed to resolve entry points.
...
* **Offline base-map tile caching** (Mapbox). The dataset cache can now download the map provider's own base-map tiles for each managed dataset, so a map renders fully offline. One region per venue is cached, covering the venue's bounds plus a buffer.
  * New `MPIMapProviderBaseMapCache` interface and `MPBaseMapCacheRegionListener` progress/completion callback.
  * `MPDataSetCacheManager.setBaseMapCacheProvider(...)` / `getBaseMapCacheProvider()` register the map adapter's provider. It is independent of any `MapControl` or map view and can be registered before a map exists.
  * `MPDataSetCacheManager.synchronizeBaseMapTiles()`, `synchronizeBaseMapTiles(listener)` and `synchronizeBaseMapTiles(dataSetCaches, listener)` start caching. The dataset's venues must already be on the device (loaded or synced).
  * `MPDataSetCacheManager.addDataSetWithCachingScope(...)` gained overloads with a `baseMapTilesEnabled` flag.
  * `MPDataSetCache.setBaseMapTilesEnabled(boolean)` / `isBaseMapTilesEnabled()` opt a dataset in, orthogonal to `MPDataSetCacheScope`. `isCachingBaseMapTiles()`, `getBaseMapTilesCachedTimestamp()` and `getBaseMapCacheSize()` report state.
  * `MPDataSetCacheManager.removeDataSetCache(...)` also removes the dataset's cached base-map regions.
  * New error codes `MIError.BASEMAP_CACHE_NOT_SUPPORTED` (9000) and `MIError.BASEMAP_CACHE_NOT_REGISTERED` (9001).
  * **MapboxAdapter:** `MPMapboxBaseMapCacheProvider(MPMapboxStyleSource)` downloads the style pack and tile region. The style is chosen explicitly via `MPMapboxStyleSource.MapsIndoorsDefault` or `MPMapboxStyleSource.Custom(styleUri)`, and must match the style the live map renders.
  * **GoogleMapsAdapter:** `MPGoogleBaseMapCacheProvider` registers like any provider but reports `BASEMAP_CACHE_NOT_SUPPORTED`, since Google Maps has no per-region offline tile API. `isBaseMapCachingSupported()` returns `false` so apps can skip the work.
* **New Insights analytics**, aligned with the web SDK. The SDK now reports which venue the user ends up viewing, which searches they settle on and how many results those return, which locations they select and whether a selection came from a search, and how routes are used from start to finish, including how much of a route was actually followed.
* `MPDirectionsRenderer.finishGuidance(usagePercentage)` lets an app report that the user finished with the current route, optionally with the percentage covered. Replacing or clearing a route completes it implicitly.
* `MPLogger.setCustomComponent(component, componentVersion)` for wrapper SDKs to identify themselves in Insights. Not intended for app developers.
* `MPIProjection.getCameraBearing()` exposes the camera bearing as part of the projection snapshot. Both adapters implement it.

### Changed

* The `Logger` class is no longer public. It was always an internal component and was never meant to be part of the public API. The one method wrapper SDKs relied on, `setCustomComponent`, now lives on `MPLogger`. Apps should not have been calling `Logger` and are unaffected.
* `MapsIndoors.load(...)`'s ready callback now fires only after the loaded locations have been ingested into the query store, so a search issued from the callback sees every location. Spatial-grid construction moved off the ready path, so a single location with a very large footprint no longer delays load or exhausts memory before ready.
* Analytics: the old map-click and online search-performed signals are replaced by the new selection and search analytics above. Offline search reporting is unchanged. A marker tap consumed by an app's `MPOnMarkerClickListener` is deliberately not reported, matching web.
* Each render pass now reads one consistent snapshot of camera, bearing, floor and max zoom taken when the pass is scheduled, instead of live map state that can change mid-pass.
* SDK worker-pool threads are named `MISDKPool-N` so they are distinguishable from app threads in a thread dump.
* **MapboxAdapter:** every MapsIndoors-owned GeoJSON source is fully replaced on each render pass rather than patched with add/remove diffs, and every content layer carries a per-floor filter so a feature can only draw on the floor it was rendered for. Mapbox source-update rejections are now logged.
* Dependency: `mapsindoors-log` 1.0.4 → 1.0.5 (required for the new analytics).

### Fixed

* **Stale floor content on Mapbox.** Icons, 2D models, walls, extrusions or floor plans from the previous floor could remain on screen after a floor change that coincided with camera movement. Three causes are fixed: render passes mixing live and stale floor state, a floor-change render trigger that could belong to a different or destroyed `MapControl`, and partial Mapbox source updates being rejected all-or-nothing while the adapter believed they had applied. Destroying an old `MapControl` no longer disarms a newer one's render trigger, and `MapProvider.clear()` now resets the renderer's tracking state. Style image ids are tracked only once Mapbox confirms the add, so a failed image load is retried instead of believed loaded forever.
* **Zoom-fit math** in the bounds capping used by `MapControl.goTo(entity, maxZoom)` and `automatedZoomLimit`: the longitude term used the cosine of the centre longitude, which produced NaN for some bounds and pinned the camera to maximum zoom. Both axes now follow Web Mercator.
* **Offline cache split by solution-id casing.** The dataset cache and the data loaders normalised the solution id differently, so a dataset synced under one casing and loaded under another used two cache folders and the offline data was not found. Both now use one locale-independent normalisation.
* The datasets manifest, and any file written directly into the SDK's root cache folder, silently failed to persist on a fresh install or after the SDK-upgrade purge, because the parent directory was never created.
* Analytics events logged concurrently from several threads could be lost; appends to the open log package are now synchronised.
* A location-source status announcement queued before `MapsIndoors.destroy()` no longer notifies observers of an instance that has already been torn down.
...
* New directions rendering. The route rendering on both Google Maps and Mapbox has been rebuilt and is now configurable at three levels:
  * `MPDirectionsRendererOptions` — a single options object applied with `MPDirectionsRenderer.setOptions()`, matching the equivalent object on Web and iOS. Covers route line color, opacity, width and stroke style (solid/dashed/dotted), animation speed (m/s) with a minimum duration and repeat flag, an independently styled animated overlay, `fitBoundsMaxZoom` for camera fitting, and `forceAnimation`.
  * `MPDirectionsRendererConfig` — full route styling applied at runtime with `MPDirectionsRenderer.setConfig()`: line and halo (casing), animation type (`FLOW`, `PULSE`, `COMET`, `NONE`) with overlay styling, repeated direction stamps along the route (five arrow presets or a custom icon, with spacing/scale/color), stop-icon styling, per-connector-type leg-boundary icons (elevator, escalator, stairs, ramp, …), start/end-marker display rules (icon, label, zoom range), camera `fitBoundsMaxZoom`, and an elevated 3D route line (Mapbox only).
  * CMS solution-level defaults — solutions on data format v8+ serve a directions renderer config with the solution (`MPSolutionConfig.getDirectionsRendererConfig()`), so route styling can be managed centrally without app code. A runtime `setConfig()` takes precedence over the solution-served config.
* Route animation now respects the system "remove animations" / reduce-motion accessibility setting (override with `forceAnimation`).
* Route animation duration are now speed-based (constant meters per second with a minimum duration) instead of a fixed duration per leg.

### Changed

* Directions rendering defaults have been modified to be equal across all platforms (Android, iOS, Web). The defaults are delivered with the solution config. The new default look is an opaque MapsIndoors-blue (`#3071D9`) route line with a subtle halo casing and a continuous "flow" animation, replacing the previous thicker semi-transparent line with an opaque animated overlay and fixed 6-second animation.
  * The old look can be restored at runtime (this wins over the solution-served config):

    ```kotlin
    mpDirectionsRenderer.setConfig(MPDirectionsRendererConfig(
        line = MPDirectionsRendererConfig.Line(strokeColor = "#3071D9", strokeOpacity = 0.5, strokeWeight = 8.0),
        ...
        animation = MPDirectionsRendererConfig.Animation(
            ...
            overlay = MPDirectionsRendererConfig.Overlay(strokeColor = "#3071D9", strokeOpacity = 1.0, strokeWeight = 5.0)
        ),
        stamp = MPDirectionsRendererConfig.Stamp(preset = MPRouteStampType.NONE),
    ))
    ```

    Note that the animation timing model is new: the fixed per-leg duration cannot be expressed in the speed-based model, so animation timing may differ slightly from previous versions.
* `MPDirectionsRenderer.setPolylineColors()` and `setAnimatedPolyline()` are deprecated in favor of `setOptions(MPDirectionsRendererOptions)`. They remain functional.
* The directions stack (\~30 classes: `MPDirectionsService` incl. online/offline, `MPDirectionsRenderer`, `MPRoute`/`MPRouteLeg`/`MPRouteStep` and the other route models, `MPDirectionsConfig`, `MPDirectionsLabel`, `MPContextualInfoSettings`, `MPSolutionConfig`) has been migrated from Java to Kotlin. The Java-facing API is unchanged. Kotlin apps may need small source adjustments where property-style access becomes an explicit getter call (e.g. `config.travelMode` → `config.getTravelMode()`).

### Fixed

* Fixed density issue for some map calculations: the map view size used for camera math was reported in dp instead of pixels, and the max-zoom bounds calculation scaled longitude by a spurious cos(longitude) term. This should result in more correct camera zoom across the board, most noticeably when fitting the camera under a zoom cap (e.g. `MPSolutionConfig` automated zoom limit, route framing) on venues far from latitude/longitude 0.
* Fixed an issue on Google Maps where 2D models would freeze in their creation state after being added to the map — subsequent updates (position, size, image, visibility) were not applied and the model could be left behind on the map.
* Fixed issue where location sources that rebuild locations with existing ids would not have those locations correctly re-added to the SDK, leaving them orphaned for some operations (e.g. filtering by category or type). Locations are now re-indexed and republished when the internal category/type mappings change.
* Fixed a rare crash (`UnsupportedOperationException`) on Google Maps when the map was torn down while a location render pass was still queued.

## \[4.18.6] 2026-08-13

### Changed

* Reworked layer ordering in the Mapbox adapter, so MapsIndoors layers hold a stable position relative to the base map and to layers added by the host app

### Fixed

* Fixed Clusters not being selectable
* Fixed issue where an `IInfoWindowAdapter` with null layout params would cause a crash on Mapbox
* Fixed layer-ordering issue on Mapbox that could cause the Positioning dot to be shown below a Route Polyline
* Fixed issue where de-selection of a location would not always propagate when selecting a new location
* Fixed Route contextual changes showing floor indexes when it should be showing floor names
* Fixed issue where buildings and venues would not be propagated correctly when restarting the SDK with an up-to-date cache
* Fixed issue where display rules were not applied to locations before those locations were handed to location listeners
* Fixed `MPDisplayRuleOptions.setModel3DRotationZ()` setting the Y rotation instead of the Z rotation
* Fixed issues where DSM would fail silently
* Fixed OOM in the DSM for large solutions
* Fixed issues where DSM would use device locale when downloading data, then be unable to match that with the allowed languages for the solution
* Fixed canonical language fallbacks for non-existing locales (eg. `EN_da` -> `EN` for a solution with language `[EN]`)
* Fixed issue where DSM was not able to sync 3D models, so they would not render for solutions cached with the DSM
* Fixed crash when reducing a dataset cache's scope before it had ever synced
* Fixed area calculations for `MPMultiPolygon` adding the area of holes instead of detracting it
* Fixed issues with `MPMultiPolygon.isInside()`
* Fixed bounding boxes derived for polygon geometries being wrong in the southern and western hemispheres

## \[4.18.5] 2026-07-17

* Fixed an issue where under specific circumstances, a building outline could be orphaned from the rendering pipeline and stick around after it should have been removed
* Fixed an issue where the default `MPFloorSelector` would behave strangely across different lifecycles

## \[4.18.4] 2026-07-07

### Changed

* Migrated Google Maps routing to the newer Routes API with automatic fallback support for improved reliability.
* Improved entry-point ranking by making it travel-mode aware. Routing now considers the selected travel mode when calculating route matrices.

### Deprecated

* MapsIndoors.isAPIKeyValid() deprecated (since 4.18.4).

### Fixed

* Fixed an issue custom marker/label disappearing on Mapbox.
* Fixed TLS security by replacing the permissive TLS client with proper certificate validation.
...
* Fixed an issue where height and width on some images would be reversed
* Fixed padding not being applied uniformly, resulting in some actions, like building detection, not respecting the set padding
  * Also fixed instances where some actions would partially override the map padding
* Fixed a crash that could occur when shutting down or reloading the SDK
* Fixed an issue where launching the SDK multiple tiles in quick succession would cause synchronization issues
* Fixed issue where `onMapsIndoorsReady` listeners set before `MapsIndoors.load()` was called would be removed
* Fixed issue where `MPFilter.setTypes()` would not function properly
* Fixed issue on Google Maps where the floor outline would stick around when switching floors

## \[4.18.2] 2026-05-18

### Fixed

* Fixed an issue where wall geometry (2D and 3D) would not be rendered for the selected building

## \[4.18.1] 2026-05-13

### Added

* Added `MPSolution.isClipLayerEnabled()`, letting app code branch on whether the clip layer module is active

### Fixed

* Fixed issue on **Mapbox** where markers would in some cases be elevated by invisible Mapbox 3D buildings
* Fixed issue on **Google Maps** where a polygon rendering error could occur if ClipLayer was active

## \[4.18.0] 2026-05-08

### Added

* Support for non-Latin languages has been improved throughout the SDK

### Changed

* Automatic Building selection is now **floor-aware**, switching to the most relevant building when multiple buildings overlap on different floors
* **Label rendering improvements on Google Maps:** POI and route-stop labels now render correctly in non-Latin scripts. Bidirectional text, complex shaping, and system font fallbacks (Noto) are handled properly, and bold labels use a real bold typeface for better quality in CJK and Arabic

### Fixed

* Fixed a `NullPointerException` when calculating floor names for route steps that have a missing start or end location
* Fixed a memory issue in `MPLocationService` which would cause elevated memory usage on large solutions
* Fixed duplicate content-synchronization calls that could occur when no data was available to sync, causing a load failure
* Fixed `InfoWindow` rendering failures
  * Fixed `MPMapConfig.Builder.setShowInfoWindowOnLocationClicked()` value being ignored on Google Maps
* Fixed several language-resolution issues affecting directions and dataset caching
* On Google Maps and Mapbox: `MPMapConfig.Builder` setters now return the correct builder type, restoring fluent method chaining

## \[4.17.3] 2026-04-28

### Changed

* `BUILDING_OUTLINE` now traces the active floor instead of the full building footprint, aligned with other platforms.
* The cache purge that runs after an SDK version upgrade no longer blocks the UI thread.

### Deprecated

* `MPAppConfig.APP_SETTING_DEFAULT_VENUE` — replaced by `APP_SETTING_VENUE`, which resolves against `administrativeId`. Existing configurations continue to work via a fallback.

### Fixed

* Custom-source POIs whose venue id does not resolve to a loaded venue are now rendered. Previously, POIs supplied by a custom `MPLocationSource` with a typo'd, stale, or absent venue id would silently fail to appear.
* Removing one location that has an external id no longer drops sibling locations sharing the same external id from the lookup cache.
* The dataset cache no longer remains in a stale state after an SDK version upgrade.
* Custom floor selectors whose view is owned by the host (e.g. declared in an XML layout) are no longer detached from the host's view tree when `MapControl` is recreated, eliminating an associated crash.
* Concurrent location lookups, updates, and teardown can no longer race, removing a class of intermittent inconsistent query results and shutdown crashes.
* On Google, POIs on a newly selected floor now appear immediately, rather than only after a subsequent camera movement.
* Rapid floor changes no longer drop render updates, so the map no longer gets stuck showing stale POIs after flipping floors quickly.
* On Mapbox, a `NoSuchElementException` crash that could occur when a location was selected while the map style was reloading or a view annotation was being detached was fixed.
* Resolved long-standing Mapbox style-handling warnings logged at runtime.
