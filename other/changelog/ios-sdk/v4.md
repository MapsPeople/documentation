...
* Base-map tiles can now be cached for offline use, so the outdoor map around a solution's venues still renders with no network \[Mapbox only]:
  ...
  * Map providers supply this through the new `MapProviderBaseMapCache` protocol, with `MPMapboxStyleSource` selecting the MapsIndoors default style or a custom one.
...
* The raw values of `MPError.unknownError`, `MPError.requestTimedOut` and `MPError.requestCancelled` have changed. Objective-C and cross-platform consumers that compare `NSError.code` against a numeric literal must be updated to compare against the `MPError` case instead.
...
* `MPFloorCodable.geometry` is now optional, so a floor delivered with a single polygon geometry no longer fails to decode \[MapsIndoors Codable only].
* Hiding base-map 3D objects through `MPFeatureType.extrudedBuildings` now fades only the base map's extruded buildings while online, leaving its 3D landmarks and trees visible. Offline it continues to hide all of the base map's 3D objects \[Mapbox only].

#### Fixed

* Locations no longer disappear, and the map no longer crashes, when the backend sends a numeric location field as a string \[Mapbox only].
* Restored the remaining tolerance for numeric values sent as strings, and strings sent as numbers, across the data and routing decoders. A malformed location settings block no longer discards the whole location type.
* A single unreadable field in the CMS solution configuration no longer discards the entire configuration. Each setting is now read on its own, and a setting that cannot be read is reported.
* Display rules configured in the CMS are no longer discarded by `MPDisplayRule.reset()`, and a display rule whose payload cannot be read is now reported instead of appearing as one that was never configured. A nested block arriving without its colors no longer empties the whole rule.
