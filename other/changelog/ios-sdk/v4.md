...
* Route styling is now configured through a single `MPDirectionsRendererOptions` object on `MPDirectionsRenderer.options`. It covers the route line — colour, opacity, weight, and a solid, dashed or dotted stroke style — an optional halo around it, and a separately styled overlay that animates along the route as `flow`, `pulse` or `comet` (or `none`). It also adds repeating direction indicators along the line, from five built-in arrow shapes or a custom image; custom start and destination markers with their own icons and labels; per-connector-type icons at floor changes; a cap on how far a route fit zooms in; and a 3D elevated line \[Mapbox only]. Route animation respects the system Reduce Motion setting unless `forceAnimation` is set. The same configuration can also be set per solution in the CMS and is applied automatically, exposed as `MPSolutionConfig.directionsRendererConfig`, with an option set in code taking precedence over the solution value, and the solution value over the SDK default.
...
* Added `MPMapControl.userPositionOpacityOverride` to opt out of dimming the user's position when it is on a floor other than the one displayed. Set a value to apply that opacity on every floor; leave it unset to keep the current behaviour.

#### Changed

* The default route appearance has changed. Previously the route drew a wide line at 30% opacity with a narrower full-colour line animating along it; it now draws a solid line with a thinner, lighter overlay travelling along it, and the halo around the line is off by default. Set the corresponding `MPDirectionsRendererOptions` properties to restore a previous look.
...
* Data providers now serve from an in-memory cache before going to the network, removing redundant requests within a session. The cache is cleared when a solution is loaded or synchronised.
* Updated the Google Maps SDK to 10.15.0 (from 9.4.0) and the Mapbox Maps SDK to 11.25.0 (from 11.18.2). The Swift Package for Google Maps now resolves an exact Google Maps version rather than a range.

#### Deprecated

* `MPDirectionsRenderer.fitBounds`, `padding`, `pathColor` and `backgroundColor` are deprecated in favour of `fitBounds`, `fitBoundsPadding`, `strokeColor` and `backgroundColor` on `MPDirectionsRendererOptions`. The existing properties continue to work.

#### Fixed

* Fixed a crash caused by concurrent access while rendering on Google Maps.
* Fixed data races in the shared native library.
* Fixed data races in the route flow animation, which now runs on a native display-synchronised animator.
* Filtered searches no longer risk blocking a thread while converting filters, which could stall other concurrent work.
* Route camera fits now honour the solution's automated zoom limit on Google Maps, which previously ignored it. Mapbox was unaffected.
